...
CREATE OR REPLACE PROCEDURE dbo."UP_insight_SessionEndpointGetBySessionId"
(
    IN       pi_UserId       INTEGER,
    INOUT    po_SessionId    UUID,
...
BEGIN
    SELECT 
        ...
        SE."DateCompleted",
        SE."CallbackToken",
        SE."MatchingMessageCount",
        SE."DateCreated",
        SE."DateAmended",
        SE."WhoAmended_nt_username",
        SE."WhoAmended_hostname",
        SER."RowsReturned" AS MessagesCount
    FROM dbo."SessionEndpoint" AS SE
    INNER JOIN dbo."Session" AS S ON S."SessionId" = SE."SessionId"
    INNER JOIN dbo."Session" AS S ON S."SessionId" = SE."SessionId"
    LEFT OUTER JOIN dbo."SessionEndpointResult" AS SER ON SE."SessionEndpointId" = SER."SessionEndpointId"
    LEFT OUTER JOIN dbo."ResultPart" AS RP ON SER."ResultPartId" = RP."ResultPartId"
    WHERE
        SE."SessionId" =po_SessionId  
        AND S."UserId" = pi_UserId 
        AND (RP."Description" = 'Messages' OR RP."Description" IS NULL);
